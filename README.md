...
```bash
./scripts/economist-0-runme.sh
```

Optional edition date:

```bash
./scripts/economist-0-runme.sh 2025-09-13
```

Individual steps (normally called by `economist-0-runme.sh`):

| Script | Step |
|--------|------|
| `economist-1-download.sh` | Download MP3 from RSS |
| `economist-2-process-edition.sh` | Split chapters, artwork, RAR |
| `economist-3-speedup-loudness.sh` | Speed + loudness (ffmpeg) |
| `economist-4-move-results.sh` | Move to `_obrobione` |

Override config path:

```bash
ECONOMIST_CONF=/path/to/my.conf ./scripts/economist-0-runme.sh
```
