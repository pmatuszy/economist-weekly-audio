...
`install.sh` is interactive: it shows the repo path, config plan, wrappers to create, and asks **Proceed with installation? [y/N/q]** before writing anything. If old numbered wrappers (`0-economist-runme.sh`, etc.) exist in `bin/`, it offers to remove them after install.
